...
$handle = fopen($file, "r");
        ...
        if($questioni !=""){
            ...
            $table = "questions";
            $key = "village";
            $questions  = $question->AESEncrypt($question->setQuestion($questioni), $key);
              $optiona = $question->AESEncrypt($question->setQuestion($optionas), $key);
              $optionb = $question->AESEncrypt($question->setQuestion($optionbs), $key);
              $optionc = $question->AESEncrypt($optioncs?$question->setQuestion($optioncs):"", $key);
              $optiond = $question->AESEncrypt($optionds?$question->setQuestion($optionds):"", $key);
              $optione = $question->AESEncrypt($optiones?$question->setQuestion($optiones):"", $key);
              $qtype = "file";
              $correctanswer = $question->AESEncrypt(ucwords($question->test_input($corrects)), $key);;
              $mark = $question->AESEncrypt($question->test_input($marks), $key);;
                 $examid = $_POST['examid'];
